...
contour = plt.contour(res, levels=equipotential_values, colors='black', linestyles='solid', extent=(0, nx-1, 0, ny-1))
plt.clabel(contour, inline=True, fontsize=8, fmt='%1.1f')

plt.show()

# Verificar se existem contornos válidos nos extremos
paths_min = contour.collections[0].get_paths()  # Equipotencial de valor -1 (menor)
paths_max = contour.collections[1].get_paths()  # Equipotencial de valor 1 (maior)

if len(paths_min) > 0 and len(paths_max) > 0:
    # Usar o primeiro caminho de cada equipotencial
    equipotential_min = paths_min[0].vertices
    equipotential_max = paths_max[0].vertices

    # Inicializar uma lista para armazenar as distâncias entre os extremos
    distances = []

    # Calcular a distância entre cada ponto de equipotential_min e o ponto mais próximo em equipotential_max
    for point in equipotential_min:
        dists = distance.cdist([point], equipotential_max, 'euclidean')  # Distância euclidiana
        min_dist = np.min(dists)  # Encontrar a menor distância
        distances.append(min_dist)

    # Converter a lista de distâncias em um array
